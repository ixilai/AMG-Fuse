...
class Compose(object):
    ...
    def __call__(self, image, target, image_gt, target_gt, fuse):
        for t in self.transforms:
            image, target, image_gt, target_gt, fuse = t(image, target, image_gt, target_gt, fuse)
        return image, target, image_gt, target_gt, fuse
...
class Resize(object):
    ...
    def __call__(self, image, target, image_gt, target_gt, fuse):
        ...
        fuse = F.resize(fuse, self.size, interpolation=T.InterpolationMode.NEAREST)
...
class Resize_16(object):
    ...
    def __call__(self, image, target, image_gt, target_gt, fuse):
        ...
        fuse = F.resize(fuse, (new_height, new_width))
...
class RandomCrop(object):
    ...
    def __call__(self, image, target, image_gt, target_gt, fuse):
        ...
        fuse= pad_if_smaller(fuse, self.size)
...
class CenterCrop(object):
    ...
    def __call__(self, image, target, image_gt, target_gt, fuse):
        ...
        fuse = F.center_crop(fuse, self.size)
        return image, target, image_gt, target_gt, fuse
